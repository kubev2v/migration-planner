...
SIZER_REPLICAS ?= 2
SIZER_PORT ?= 9200
COST_ESTIMATION_IMAGE ?= quay.io/redhat-user-workloads/assisted-migration-tenant/cost-estimation
COST_ESTIMATION_IMAGE_TAG ?= latest
COST_ESTIMATION_REPLICAS ?= 2
COST_ESTIMATION_PORT ?= 9205
COST_ESTIMATION_LOG_LEVEL ?= info
COST_ESTIMATION_IMAGE_PULL_POLICY ?= Always
COST_ESTIMATION_MIGRATION_PLANNER_URL ?= http://host.containers.internal:3443
COST_ESTIMATION_MIGRATION_PLANNER_TIMEOUT ?= 30s
COST_ESTIMATION_AUTH_TYPE ?= none
COST_ESTIMATION_SKIP_PARTNER_CHECK ?= true
...
run-ce:
	@echo "🚀 Starting cost-estimation container on port $(COST_ESTIMATION_PORT)..."
	...
	@$(PODMAN) run -d --name migration-planner-cost-estimation-local \
		...
		-e LOG_LEVEL=$(COST_ESTIMATION_LOG_LEVEL) \
		-e COST_ESTIMATION_AUTH_TYPE=$(COST_ESTIMATION_AUTH_TYPE) \
		-e COST_ESTIMATION_SKIP_PARTNER_CHECK=$(COST_ESTIMATION_SKIP_PARTNER_CHECK) \
		-e PLANNER_BASE_URL=$(COST_ESTIMATION_MIGRATION_PLANNER_URL) \
		-e PLANNER_HTTP_TIMEOUT=$(COST_ESTIMATION_MIGRATION_PLANNER_TIMEOUT) \
	...
	@echo "   Planner URL: $(COST_ESTIMATION_MIGRATION_PLANNER_URL)"
...
deploy-on-openshift: oc
	...
	oc process -f deploy/templates/service-template.yml \
       ...
       -p COST_ESTIMATION_IMAGE=$(COST_ESTIMATION_IMAGE) \
       -p COST_ESTIMATION_IMAGE_TAG=$(COST_ESTIMATION_IMAGE_TAG) \
       -p COST_ESTIMATION_REPLICAS=$(COST_ESTIMATION_REPLICAS) \
       -p COST_ESTIMATION_PORT=$(COST_ESTIMATION_PORT) \
       -p COST_ESTIMATION_IMAGE_PULL_POLICY=$(COST_ESTIMATION_IMAGE_PULL_POLICY) \
       -p COST_ESTIMATION_MIGRATION_PLANNER_URL=http://migration-planner-agent:7443$(SERVICE_API_PATH) \
       -p COST_ESTIMATION_MIGRATION_PLANNER_TIMEOUT=$(COST_ESTIMATION_MIGRATION_PLANNER_TIMEOUT) \
       -p COST_ESTIMATION_AUTH_TYPE=$(COST_ESTIMATION_AUTH_TYPE) \
       -p COST_ESTIMATION_SKIP_PARTNER_CHECK=$(COST_ESTIMATION_SKIP_PARTNER_CHECK) \
       -p MIGRATION_PLANNER_REPLICAS=${MIGRATION_PLANNER_REPLICAS} \
       -p SIZER_REPLICAS=$(SIZER_REPLICAS) \
