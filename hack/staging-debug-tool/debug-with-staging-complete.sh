...
if [ "$AUTH_MODE" = "none" ]; then
    ...
    echo "📁 Private key saved to: $(pwd)/auth_local/private-key.txt"
fi
...
if [ "$AUTH_MODE" = "local" ]; then
    ...
    cp "$SCRIPT_DIR/auth_local/set-ui-token.html" "$MIGRATION_PLANNER_UI_REPO/"
    cp "$SCRIPT_DIR/auth_local/authFetch.ts" "$MIGRATION_PLANNER_UI_REPO/"
    podman build -f "$SCRIPT_DIR/auth_local/Dockerfile.ui-local" -t "$UI_IMAGE" "$MIGRATION_PLANNER_UI_REPO"
    rm -f "$MIGRATION_PLANNER_UI_REPO/authFetch.ts" "$MIGRATION_PLANNER_UI_REPO/set-ui-token.html"
else
    # For auth=none, use standard dockerfile without authFetch
    podman build -f "$SCRIPT_DIR/auth_none/Dockerfile.ui" -t "$UI_IMAGE" "$MIGRATION_PLANNER_UI_REPO"
fi
...
echo ""
echo "🔗 Connections:"
echo "  🖥️  UI: http://localhost:3000/openshift/migration-assessment"
echo "  🌐 API: http://localhost:3443"
echo "  💡 Debugger: localhost:40000"
echo "  🗄️  Database: localhost:5432"
echo ""
if [ "$AUTH_MODE" = "local" ]; then
  echo "📊 Staging data loaded (org_ids preserved for multi-tenant debugging)"
else
