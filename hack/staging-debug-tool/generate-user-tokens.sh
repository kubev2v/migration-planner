...
if [ ! -f ".env-debug" ]; then
  echo "❌ .env-debug not found; copy from .env-debug.example and set paths."
  exit 1
fi
source .env-debug
if [ -z "${MIGRATION_PLANNER_REPO:-}" ] || [ ! -d "$MIGRATION_PLANNER_REPO" ]; then
  echo "❌ MIGRATION_PLANNER_REPO is unset or invalid: '$MIGRATION_PLANNER_REPO'"
  exit 1
fi
...
DOMAINS=$(podman exec planner-db-staging psql -U demouser -d planner -At -c "
    SELECT DISTINCT 
        CASE 
...
for domain in $DOMAINS; do
    ...
    TOKEN=""
    if ! TOKEN="$(cd "$API_REPO" && ./bin/planner sso token --private-key "$PRIVATE_KEY" --username testuser --org "$ORG_ID" 2>/dev/null)"; then
        echo "❌ Failed to generate token for domain: $domain (org: $ORG_ID)"
        continue
    fi
    if [ -z "$TOKEN" ]; then
        echo "❌ Empty token for domain: $domain (org: $ORG_ID)"
        continue
    fi
