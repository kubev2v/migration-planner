...
import (
	...
	servicepkg "github.com/kubev2v/migration-planner/test/e2e/service"
	"go.uber.org/zap"
)

// CreateAgent Create VM with the UUID of the source created
func CreateAgent(sourceId uuid.UUID, vmName string, svc servicepkg.PlannerService) (agentpkg.PlannerAgent, error) {
	...
	agent, err := agentpkg.NewPlannerAgent(vmName, url)
	if err != nil {
		return nil, err
	}
	err = agent.Run()
...
func AgentIsUpToDate(svc servicepkg.PlannerService, uuid uuid.UUID) (bool, error) {
