...
func processVMInfo(excelFile *excelize.File, sheetName string) ([]VM, error) {
...
func fillInventoryWithVMData(vms []VM, inventory *api.Inventory) {
	...
	inventory.Vms.CpuCores.Histogram = histogram(cpuSet)
	inventory.Vms.RamGB.Histogram = histogram(memorySet)
	inventory.Vms.DiskCount.Histogram = histogram(diskCountSet)
	inventory.Vms.DiskGB.Histogram = histogram(diskGBSet)
