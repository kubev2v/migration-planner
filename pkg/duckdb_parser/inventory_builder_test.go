...
func TestBuildInventory_StandaloneHostDetection(t *testing.T) {
	cases := []struct {
		...
		{
			name: "RVTools standalone-host VM placeholder",
			vms: []map[string]string{
				{"VM": "vm-1", "VM ID": "vm-001", "VI SDK UUID": "uuid-1", "Host": "esxi-host-1", "CPUs": "4", "Memory": "8192", "Powerstate": "poweredOn", "Cluster": "cluster1", "Datacenter": "dc1"},
			},
			hosts: []map[string]string{
				{"Datacenter": "dc1", "Cluster": "cluster1", "# Cores": "8", "# CPU": "2", "Object ID": "host-001", "# Memory": "32768", "Model": "ESXi", "Vendor": "VMware", "Host": "esxi-host-1", "Config status": "green"},
				// DuckDB reads RVTools' empty shared-string cell as "VM".
				{"Datacenter": "dc1", "Cluster": "VM", "# Cores": "8", "# CPU": "2", "Object ID": "host-002", "# Memory": "32768", "Model": "ESXi", "Vendor": "VMware", "Host": "esxi-host-2", "Config status": "green"},
			},
			vClusters: []map[string]string{{"Name": "cluster1", "Object ID": "domain-c1", "DRS enabled": "false", "HA enabled": "false"}},
			want:      true,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			sheets := defaultStandardSheets(tc.vms, tc.hosts)
			if tc.vClusters != nil {
				sheets[3].Rows = tc.vClusters
			}
...
func TestBuildInventory_VMsWithFaultToleranceCount(t *testing.T) {
	...
	// VM list filter: vm-001 is FT-enabled; vm-002 is not.
	countVMList, err := parser.VMsWithFaultToleranceCount(ctx, Filters{VMList: []string{"vm-001", "vm-002"}})
	require.NoError(t, err)
	assert.Equal(t, 1, countVMList, "VM list filter includes only FT-enabled VMs in the requested list")
...
func TestBuildInventory_VMotionSupport(t *testing.T) {
	...
	hosts := []map[string]string{
		{"Datacenter": "dc1", "Cluster": "cluster1", "# Cores": "8", "# CPU": "2", "Object ID": "host-001", "# Memory": "32768", "Model": "ESXi", "Vendor": "VMware", "Host": "esxi-host-1", "Config status": "green",
			"VMotion support": "true", "Storage VMotion support": "true"},
	}
	...
	require.NotNil(t, host.VMotionSupported, "VMotion should not be nil")
	assert.True(t, *host.VMotionSupported, "VMotion should be true")
	require.NotNil(t, host.StorageVMotionSupported, "Storage VMotion should not be nil")
	assert.True(t, *host.StorageVMotionSupported, "Storage VMotion should be true")
	...
	for _, clusterData := range inv.Clusters {
		...
		require.NotNil(t, clusterHost.VMotionSupported)
		assert.True(t, *clusterHost.VMotionSupported)
		require.NotNil(t, clusterHost.StorageVMotionSupported)
		assert.True(t, *clusterHost.StorageVMotionSupported)
	}
}
...
func TestBuildInventory_VMotionSupport_DefaultWhenMissing(t *testing.T) {
	...
	require.NotEmpty(t, inv.VCenter.Infra.Hosts)
	host := inv.VCenter.Infra.Hosts[0]
	require.NotNil(t, host.VMotionSupported, "should not be nil when column missing")
	assert.False(t, *host.VMotionSupported, "should default to false when column missing")
	require.NotNil(t, host.StorageVMotionSupported, "should not be nil when column missing")
