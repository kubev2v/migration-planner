...
var _ = Describe("agent store", Ordered, func() {
	...
	Context("delete", func() {
		It("successfuly delete an agent", func() {
			source1 := uuid.NewString()
			tx := gormdb.Exec(fmt.Sprintf(insertSourceStm, source1, "source-1"))
			Expect(tx.Error).To(BeNil())
			...
			Expect(tx.Error).To(BeNil())
			agent1ID := uuid.New()
			tx = gormdb.Exec(fmt.Sprintf(insertAgentStm, agent1ID, "not-connected", "status-info-2", "cred_url-2", source2))
			Expect(tx.Error).To(BeNil())

			err := s.Agent().Delete(context.TODO(), agentID)
			Expect(err).To(BeNil())
