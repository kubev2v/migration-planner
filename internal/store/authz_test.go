...
//    - Tests direct user relationships with assessments (owner, reader)
//    - Coverage: WithOwnerRelationship, WithReaderRelationship
//    - Validates: Owner has all permissions, reader has read-only access
...
//    - Validates:
//      * Single permissions: owner, reader, org member, platform roles
//      * Combined permissions: owner+org, reader+org
...
var _ = Describe("AuthzStore", Ordered, func() {
	...
	Context("Write Relationships", func() {
		...
		It("should write multiple relationships atomically", func() {
			...
			err := authzSvc.WriteRelationships(ctx,
				...
				store.WithReaderRelationship(batchAssessment2ID, userSubject),
		...
		// Test: Validates removing reader relationship
		// Expected: Reader access should be revoked
		// Purpose: Tests WithoutReaderRelationship function
		It("should remove reader relationship using WithoutReaderRelationship", func() {
			readerRemoveUserID := "reader-remove-" + uuid.New().String()[:8]
			readerRemoveOrgID := "reader-remove-org-" + uuid.New().String()[:8]
			readerRemoveAssessmentID := "reader-remove-assessment-" + uuid.New().String()[:8]

			_, err := setupUserOrganizationMembership(ctx, spiceDBClient, zedTokenStore, readerRemoveUserID, readerRemoveOrgID)
			Expect(err).To(BeNil())

			userSubject := model.NewUserSubject(readerRemoveUserID)

			// Create reader relationship
			err = authzSvc.WriteRelationships(ctx, store.WithReaderRelationship(readerRemoveAssessmentID, userSubject))
			Expect(err).To(BeNil())

			// Verify user has read permission
			permissionsMap, err := authzSvc.GetPermissions(ctx, []string{readerRemoveAssessmentID}, readerRemoveUserID)
			Expect(err).To(BeNil())
			Expect(permissionsMap[readerRemoveAssessmentID]).To(ContainElement(model.ReadPermission))

			// Remove reader relationship
			err = authzSvc.WriteRelationships(ctx, store.WithoutReaderRelationship(readerRemoveAssessmentID, userSubject))
			Expect(err).To(BeNil())

			// Verify user has no permissions
			permissionsMap, err = authzSvc.GetPermissions(ctx, []string{readerRemoveAssessmentID}, readerRemoveUserID)
			Expect(err).To(BeNil())
			if permissions, exists := permissionsMap[readerRemoveAssessmentID]; exists {
		...
		// Purpose: Tests that owner/reader/org relationships are persisted
	...
	Context("List Permissions", func() {
		...
		It("should grant all permissions to platform super_admin", func() {
			...
			err := authzSvc.WriteRelationships(ctx, store.WithPlatformRelationship(platformID, subjects))
			Expect(err).To(BeNil())
	...
	Context("Delete Relationships", func() {
		...
		It("should delete all tracked relationships from database without assessment ID", func() {
			...
			err = authzSvc.WriteRelationships(ctx,
				...
				store.WithReaderRelationship(bulkDeleteAssessment2ID, user2Subject),
			)
			Expect(err).To(BeNil())
	...
	Context("Database Relationship Tracking", func() {
		...
		It("should NOT track platform relationships in database", func() {
			...
			err := authzSvc.WriteRelationships(ctx,
				store.WithPlatformRelationship(platformID, subjects),
	...
	Context("Get Permissions", func() {
		...
		// Test: Validates that GetPermissions correctly retrieves limited permissions for readers
		// Expected: Reader should only have read permission
		// Purpose: Tests permission retrieval for read-only access
		It("should return only read permission for assessment reader", func() {
			readerUserID := "perm-reader-" + uuid.New().String()[:8]
			readerOrgID := "perm-org-" + uuid.New().String()[:8]
			readerAssessmentID := "perm-reader-assessment-" + uuid.New().String()[:8]

			_, err := setupUserOrganizationMembership(ctx, spiceDBClient, zedTokenStore, readerUserID, readerOrgID)
			Expect(err).To(BeNil())

			// Create reader relationship
			userSubject := model.NewUserSubject(readerUserID)
			err = authzSvc.WriteRelationships(ctx, store.WithReaderRelationship(readerAssessmentID, userSubject))
			Expect(err).To(BeNil())

			// Get permissions
			permissionsMap, err := authzSvc.GetPermissions(ctx, []string{readerAssessmentID}, readerUserID)
			Expect(err).To(BeNil())
			Expect(permissionsMap).To(HaveKey(readerAssessmentID))

			permissions := permissionsMap[readerAssessmentID]
		...
		It("should return permissions for multiple assessments", func() {
			...
			err = authzSvc.WriteRelationships(ctx,
				...
				store.WithReaderRelationship(assessment2ID, userSubject),
			)
			Expect(err).To(BeNil())
			...
			// Verify assessment2 - reader permissions
		...
		// Test: Validates combined permissions from reader and org member
		// Expected: User should get read from reader + read/edit from org = read/edit
		// Purpose: Tests permission union from multiple sources
		It("should combine permissions from reader and org member roles", func() {
			readerOrgUserID := "perm-reader-org-" + uuid.New().String()[:8]
			readerOrgID := "perm-reader-org-id-" + uuid.New().String()[:8]
			readerOrgAssessmentID := "perm-reader-org-assessment-" + uuid.New().String()[:8]

			_, err := setupUserOrganizationMembership(ctx, spiceDBClient, zedTokenStore, readerOrgUserID, readerOrgID)
			Expect(err).To(BeNil())

			userSubject := model.NewUserSubject(readerOrgUserID)
			orgSubject := model.NewOrganizationSubject(readerOrgID)

			// Create both reader and org relationships
			err = authzSvc.WriteRelationships(ctx,
				store.WithReaderRelationship(readerOrgAssessmentID, userSubject),
				store.WithOrganizationRelationship(readerOrgAssessmentID, orgSubject),
			)
			Expect(err).To(BeNil())

			// Get permissions
			permissionsMap, err := authzSvc.GetPermissions(ctx, []string{readerOrgAssessmentID}, readerOrgUserID)
			Expect(err).To(BeNil())
			Expect(permissionsMap).To(HaveKey(readerOrgAssessmentID))

			permissions := permissionsMap[readerOrgAssessmentID]
			Expect(permissions).To(ContainElements(
				model.ReadPermission,
				model.EditPermission,
...
func verifyUserOrganizationMembership(ctx context.Context, client *authzed.Client, zedStore *store.ZedTokenStore, userID, orgID string) error {
	...
	fmt.Printf("%s %s", hash(orgID), hash(userID))
