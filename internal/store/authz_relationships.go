...
// Reader relationships grant read-only access to an assessment (read permission only).
//
// Parameters:
...
// Note:
//   - Returns RelationshipOpUpdate (tracked in database)
//   - Only accepts User subjects; Organization subjects are not valid for reader relation
//
// Example:
//
//	userSubject := model.NewUserSubject("user456")
//	err := authzService.WriteRelationships(ctx, WithReaderRelationship("assessment789", userSubject))
//	if err != nil {
...
func WithReaderRelationship(assessmentID string, subject model.Subject) model.RelationshipFn {
	return func(updates []*v1pb.RelationshipUpdate) ([]*v1pb.RelationshipUpdate, model.Relationship, model.RelationshipOp) {
		...
		relationship := model.NewRelationship(assessmentID, subject, model.ReaderRelationshipKind)
...
//	}
func WithPlatformRelationship(platformID string, subjects map[string][]model.Subject) model.RelationshipFn {
	return func(updates []*v1pb.RelationshipUpdate) ([]*v1pb.RelationshipUpdate, model.Relationship, model.RelationshipOp) {
...
// WithoutReaderRelationship creates a relationship function that removes a reader relationship.
...
//
//	userSubject := model.NewUserSubject("user456")
//	err := authzService.WriteRelationships(ctx, WithoutReaderRelationship("assessment789", userSubject))
//	if err != nil {
//	    log.Printf("Failed to remove read access: %v", err)
//	}
func WithoutReaderRelationship(assessmentID string, subject model.Subject) model.RelationshipFn {
	return func(updates []*v1pb.RelationshipUpdate) ([]*v1pb.RelationshipUpdate, model.Relationship, model.RelationshipOp) {
		relationshipUpdate := &v1pb.RelationshipUpdate{
			Operation: v1pb.RelationshipUpdate_OPERATION_DELETE,
			Relationship: &v1pb.Relationship{
