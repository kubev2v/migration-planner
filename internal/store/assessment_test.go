...
var _ = Describe("assessment store", Ordered, func() {
	...
	Context("update", func() {
		It("successfully updates assessment name", func() {
			...
			updated, err := s.Assessment().UpdateName(context.TODO(), assessmentID, newName)
			Expect(err).To(BeNil())
		...
		It("successfully adds new snapshot to assessment", func() {
			...
			err = s.Assessment().AddSnapshot(context.TODO(), assessmentID, &inventory2)
			Expect(err).To(BeNil())
		...
		It("fails to update non-existent assessment", func() {
			...
			_, err := s.Assessment().UpdateName(context.TODO(), nonExistentID, newName)
			Expect(err).ToNot(BeNil())
			Expect(err).To(Equal(store.ErrRecordNotFound))
		})
