...
var _ = Describe("source store", Ordered, func() {
	...
	Context("labels", func() {
		...
		It("deletes share token when source is deleted (CASCADE)", func() {
			...
			tx = gormdb.Exec(fmt.Sprintf("INSERT INTO share_tokens (id, token, source_id) VALUES ('%s', '%s', '%s');", shareTokenID, "test-token", sourceID))
			Expect(tx.Error).To(BeNil())
		...
		It("deletes only the correct share token when multiple sources exist", func() {
			sourceID1 := uuid.New()
			sourceID2 := uuid.New()
			shareTokenID1 := uuid.New()
			shareTokenID2 := uuid.New()
			...
			tx = gormdb.Exec(fmt.Sprintf("INSERT INTO share_tokens (id, token, source_id) VALUES ('%s', '%s', '%s');", shareTokenID1, "test-token-1", sourceID1))
			Expect(tx.Error).To(BeNil())
			tx = gormdb.Exec(fmt.Sprintf("INSERT INTO share_tokens (id, token, source_id) VALUES ('%s', '%s', '%s');", shareTokenID2, "test-token-2", sourceID2))
			Expect(tx.Error).To(BeNil())
