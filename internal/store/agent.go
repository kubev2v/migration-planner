...
func (a *AgentStore) InitialMigration() error {
	return a.db.AutoMigrate(&model.Agent{})
}
...
func (a *AgentStore) List(ctx context.Context, filter *AgentQueryFilter, opts *AgentQueryOptions) (*api.AgentList, error) {
	...
	apiAgentList := agents.ToApiResource()
	return &apiAgentList, nil
}
...
func (a *AgentStore) Create(ctx context.Context, agentUpdate apiAgent.AgentStatusUpdate) (*api.Agent, error) {
	...
	if err := a.db.WithContext(ctx).Create(agent).Error; err != nil {
		return nil, err
	}
...
func (a *AgentStore) Update(ctx context.Context, agentUpdate apiAgent.AgentStatusUpdate) (*api.Agent, error) {
	...
	if err := a.db.WithContext(ctx).First(&model.Agent{ID: agentUpdate.Id}).Error; err != nil {
		return nil, err
	}

	if tx := a.db.WithContext(ctx).Clauses(clause.Returning{}).Updates(&agent); tx.Error != nil {
		return nil, tx.Error
	}
...
func (a *AgentStore) UpdateSourceID(ctx context.Context, agentID string, sourceID string) (*api.Agent, error) {
	...
	if err := a.db.WithContext(ctx).First(agent).Error; err != nil {
		return nil, err
	}
