...
func (w *RVToolsWorker) Work(ctx context.Context, job *river.Job[RVToolsJobArgs]) error {
	...
	if err != nil {
		...
		if updateErr := w.store.Snapshot().UpdateStatus(ctx, job.Args.SnapshotID, model.SnapshotStatusFailed, &errMsg); updateErr != nil {
	...
	if err := w.store.Snapshot().UpdateStatus(ctx, job.Args.SnapshotID, model.SnapshotStatusParsing, nil); err != nil {
	...
	if err := w.store.Snapshot().UpdateInventory(ctx, job.Args.SnapshotID, *inventory); err != nil {
		errMsg := fmt.Sprintf("error updating snapshot inventory: %v", err)
		logger.Error(err).WithString("step", "update_inventory").Log()
		if updateErr := w.store.Snapshot().UpdateStatus(ctx, job.Args.SnapshotID, model.SnapshotStatusFailed, &errMsg); updateErr != nil {
			logger.Error(updateErr).WithString("step", "update_status_failed").Log()
		}
	...
	logger.Step("inventory_updated").Log()

	// Update snapshot status to ready
	if err := w.store.Snapshot().UpdateStatus(ctx, job.Args.SnapshotID, model.SnapshotStatusReady, nil); err != nil {
		logger.Error(err).WithString("step", "update_status_ready").Log()
