...
var _ = Describe("RVTools Background Processing", Ordered, func() {
	...
	Describe("Error handling", func() {
		It("handles invalid base64 data", func() {
			...
			// Create assessment with nil inventory (pending snapshot)
			createdAssessment, err := s.Assessment().Create(ctx, assessment, nil)
			Expect(err).To(BeNil())
			...
			Expect(*snapshot.Error).To(ContainSubstring("base64"))
		})
