...
var _ = Describe("partner handler", Ordered, func() {
	...
	Context("CreatePartnerRequest", func() {
		It("creates a request for a regular user", func() {
			...
			Expect(created.RequestStatus).To(Equal(api.PartnerRequestRequestStatusAwaiting))
		})

		It("returns 400 when user is not regular (partner member)", func() {
			partnerGroupID := uuid.New()
			tx := gormdb.Exec(fmt.Sprintf(insertPartnerHandlerGroupStm, partnerGroupID, "Partner Org", "desc", "partner", "icon", "Acme", "NULL"))
			Expect(tx.Error).To(BeNil())
	...
	Context("CancelPartnerRequest", func() {
		It("cancels a pending request", func() {
			...
			var count int
			tx = gormdb.Raw("SELECT COUNT(*) FROM partners_customers WHERE username = 'user1';").Scan(&count)
			Expect(tx.Error).To(BeNil())
	...
	Context("GetPartner", func() {
		...
		It("returns 404 when user is not a customer of this partner", func() {
			...
			Expect(err).To(BeNil())
			Expect(reflect.TypeOf(resp)).To(Equal(reflect.TypeOf(server.GetPartner404JSONResponse{})))
		})
	...
	Context("LeavePartner", func() {
		It("removes partner relationship", func() {
			...
			var count int
			tx = gormdb.Raw("SELECT COUNT(*) FROM partners_customers WHERE username = 'customer1';").Scan(&count)
			Expect(tx.Error).To(BeNil())
			Expect(count).To(Equal(0))
		})
	...
	Context("ListCustomers", func() {
		...
		It("returns 400 when user is not a partner", func() {
			authUser := auth.User{Username: "regularuser", Organization: "org"}
			ctx := auth.NewTokenContext(context.TODO(), authUser)

			resp, err := srv.ListCustomers(ctx, server.ListCustomersRequestObject{})
			Expect(err).To(BeNil())
			Expect(reflect.TypeOf(resp)).To(Equal(reflect.TypeOf(server.ListCustomers400JSONResponse{})))
		})
	...
	Context("UpdatePartnerRequest", func() {
		It("accepts a customer request", func() {
			...
			body := api.PartnerRequestUpdate{Status: api.PartnerRequestUpdateStatusAccepted}
			...
			Expect(updated.RequestStatus).To(Equal(api.PartnerRequestRequestStatusAccepted))
		})
		...
		It("rejects a request with reason", func() {
			...
			reason := "Not a fit"
			body := api.PartnerRequestUpdate{Status: api.PartnerRequestUpdateStatusRejected, Reason: &reason}
			resp, err := srv.UpdatePartnerRequest(ctx, server.UpdatePartnerRequestRequestObject{
				Id:   requestID,
				Body: &body,
			})
			...
			updated := resp.(server.UpdatePartnerRequest200JSONResponse)
			Expect(updated.RequestStatus).To(Equal(api.PartnerRequestRequestStatusRejected))
		...
		It("returns 400 when rejecting without reason", func() {
			...
			body := api.PartnerRequestUpdate{Status: api.PartnerRequestUpdateStatusRejected}
			resp, err := srv.UpdatePartnerRequest(ctx, server.UpdatePartnerRequestRequestObject{
				Id:   requestID,
				Body: &body,
			})
		...
		It("returns 400 when non-partner user updates request", func() {
			requestID := uuid.New()
	...
	Context("RemoveCustomer", func() {
		It("removes a customer from partner org", func() {
			...
			var count int
			tx = gormdb.Raw("SELECT COUNT(*) FROM partners_customers WHERE username = 'cust1';").Scan(&count)
			Expect(tx.Error).To(BeNil())
			Expect(count).To(Equal(0))
		})
