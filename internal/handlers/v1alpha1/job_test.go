...
var _ = Describe("job handler", Ordered, func() {
	...
	Context("GetJob", func() {
		It("returns 404 when job not found", func() {
			...
			srv := handlers.NewServiceHandler(service.NewSourceService(s, nil), service.NewAssessmentService(s, nil), service.NewJobService(s, nil), service.NewSizerService(sizerClient, s), nil, nil)
