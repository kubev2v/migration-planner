	...
	for {
		...
		case "name":
			...
			if err != nil {
				...
				return server.CreateRVToolsAssessment400JSONResponse{Message: fmt.Sprintf("failed to read name: %v", err), RequestId: requestid.FromContextPtr(ctx)}, nil
			}
		...
		case "file":
			...
			if err != nil {
				part.Close()
				logger.Error(err).WithString("step", "read_file").Log()
				return server.CreateRVToolsAssessment400JSONResponse{Message: fmt.Sprintf("failed to read file: %v", err), RequestId: requestid.FromContextPtr(ctx)}, nil
			}
			if n == 0 {
				part.Close()
				logger.Error(fmt.Errorf("empty file")).WithString("step", "read_file").Log()
				return server.CreateRVToolsAssessment400JSONResponse{Message: "rvtools file is empty", RequestId: requestid.FromContextPtr(ctx)}, nil
			}
