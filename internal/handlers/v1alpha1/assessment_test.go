...
var _ = Describe("assessment handler", Ordered, func() {
	...
	Context("list assessments", func() {
		It("lists assessments returned by the service", func() {
			...
			Expect(tx.Error).To(BeNil())
			tx = gormdb.Exec(fmt.Sprintf(insertAssessmentStm, assessmentID3.String(), "assessment3", "batman", "batman", "Bruce", "Wayne", service.SourceTypeInventory, "NULL"))
			Expect(tx.Error).To(BeNil())
			...
			Expect(assessmentList).To(HaveLen(3))
			Expect(assessmentList).To(ContainElement(HaveField("Id", assessmentID1)))
			Expect(assessmentList).To(ContainElement(HaveField("Id", assessmentID2)))
			Expect(assessmentList).To(ContainElement(HaveField("Id", assessmentID3)))
		})

		It("returns an empty list when no assessments exist", func() {
			user := auth.User{
				Username:     "empty",
				Organization: "empty",
		...
		It("lists assessments without sourceId filtering", func() {
	...
	Context("create assessment", func() {
		...
		It("fails to create assessment with sourceID owned by different user", func() {
			// Create a source for different user
			sourceID := uuid.New()
	...
	Context("get assessment", func() {
		...
		It("returns 403 for assessment owned by different user", func() {
			...
			srv := handlers.NewServiceHandler(service.NewSourceService(s, nil), &ForbiddenAssessmentService{}, nil, service.NewSizerService(nil, s), nil, nil, nil, nil)
			resp, err := srv.GetAssessment(ctx, server.GetAssessmentRequestObject{Id: assessmentID})
			Expect(err).To(BeNil())
	...
	Context("update assessment", func() {
		...
		It("successfully updates assessment created from agent sourceType and creates new snapshot", func() {
			...
			assessmentID := uuid.New()
			tx = gormdb.Exec(fmt.Sprintf("INSERT INTO assessments (id, created_at, name, username, org_id, owner_first_name, owner_last_name, source_type, source_id) VALUES ('%s', now(), '%s', '%s', '%s', '%s', '%s', '%s', '%s');",
