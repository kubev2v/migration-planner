...
var _ = Describe("assessment handler", Ordered, func() {
	...
	Context("list assessments", func() {
		It("successfully lists all assessments for the user (private to username)", func() {
			...
			srv := handlers.NewServiceHandler(service.NewSourceService(s, nil), service.NewAssessmentService(s, nil), nil, service.NewSizerService(nil, s), nil, nil)
		...
		It("returns empty list when no assessments exist for the user", func() {
			user := auth.User{
				Username:     "empty",
				Organization: "empty",
	...
	Context("get assessment", func() {
		...
		It("returns 403 for assessment owned by different user", func() {
			...
			srv := handlers.NewServiceHandler(service.NewSourceService(s, nil), &ForbiddenAssessmentService{}, nil, service.NewSizerService(nil, s), nil, nil)
			resp, err := srv.GetAssessment(ctx, server.GetAssessmentRequestObject{Id: assessmentID})
			Expect(err).To(BeNil())
