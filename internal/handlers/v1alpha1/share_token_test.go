...
var _ = Describe("share token handler", Ordered, func() {
	...
	Context("CreateShareToken", func() {
		It("successfully creates a new share token", func() {
			...
			Expect(shareToken.OrgId).To(Equal("admin"))
		...
		It("returns existing share token when one already exists", func() {
			...
			tokenID := uuid.New()
			tx = gormdb.Exec(fmt.Sprintf(insertShareTokenStm, tokenID, existingToken, sourceID, "admin"))
			Expect(tx.Error).To(BeNil())
	...
	Context("DeleteShareToken", func() {
		It("successfully deletes an existing share token", func() {
			...
			// Insert share token
			tokenID := uuid.New()
			tx = gormdb.Exec(fmt.Sprintf(insertShareTokenStm, tokenID, tokenValue, sourceID, "admin"))
			Expect(tx.Error).To(BeNil())
	...
	Context("GetSharedSource", func() {
		...
		It("allows access to sources from different organizations via token", func() {
			...
			// Insert share token
			tokenID := uuid.New()
			tx = gormdb.Exec(fmt.Sprintf(insertShareTokenStm, tokenID, tokenValue, sourceID, "secret-org"))
			Expect(tx.Error).To(BeNil())
