...
func (s *SizerService) CalculateClusterRequirements(
	...
	// Validate VM limit not exceeded
	if maxVMsPerNode > MaxVMsPerWorkerNode {
		err := NewErrInvalidRequest(fmt.Sprintf("VM distribution constraint violated: found %d VMs on a worker node, exceeds limit of %d per node",
...
// BuildServiceAvoidLists groups services to enforce MaxVMsPerWorkerNode limit.
// Returns avoid lists preventing different groups from colocating.
