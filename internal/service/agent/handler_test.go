...
var _ = Describe("agent service", Ordered, func() {
	...
	Context("Update agent status", func() {
		It("successfully creates the agent", func() {
			...
			// source should be associated with this agent
			associatedAgentID := ""
			tx = gormdb.Raw(fmt.Sprintf("SELECT associated_agent_id from sources WHERE id = '%s';", sourceID)).Scan(&associatedAgentID)
			Expect(tx.Error).To(BeNil())
			Expect(associatedAgentID).To(Equal(agentID.String()))
