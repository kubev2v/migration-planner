...
func (s *ShareTokenService) CreateShareToken(ctx context.Context, sourceID uuid.UUID, orgID string) (*model.ShareToken, error) {
	...
	shareToken := model.NewShareToken(sourceID, orgID, token)
	...
	if err != nil {
		...
		// Check if there's now an existing token for this source
		existingToken, getErr := s.store.ShareToken().GetBySourceID(ctx, sourceID)
		if getErr == nil {
			// A token was created by another request, return it
			return existingToken, nil
		}
		// If we still can't find an existing token, the original error was something else
...
func (s *ShareTokenService) GetSourceByToken(ctx context.Context, token string) (*model.Source, error) {
	...
	if err != nil {
		if errors.Is(err, store.ErrRecordNotFound) {
			return nil, &ErrResourceNotFound{fmt.Errorf("share token not found")}
		}
		return nil, fmt.Errorf("failed to get source by token: %w", err)
	}
