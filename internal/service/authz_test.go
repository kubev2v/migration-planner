...
//    - Coverage: owner, reader, org member, platform roles
...
var _ = Describe("Authz Service", Ordered, func() {
	...
	Context("ListAssessments", func() {
		...
		// Test: Validates user can list assessments shared as reader
		// Expected: Should return assessments where user has reader access
		// Purpose: Tests reader-based discovery
		It("should list assessments where user is reader", func() {
			ownerID := "list-reader-owner-" + uuid.New().String()[:8]
			readerID := "list-reader-user-" + uuid.New().String()[:8]
			orgID := "list-reader-org-" + uuid.New().String()[:8]
			assessmentID := uuid.New().String()

			owner := auth.User{Username: ownerID, Organization: orgID}
			reader := auth.User{Username: readerID, Organization: orgID}

			// Create users
			err := authzSvc.CreateUser(ctx, owner)
			Expect(err).To(BeNil())
			err = authzSvc.CreateUser(ctx, reader)
			Expect(err).To(BeNil())
			...
			// Share with reader
			err = authzSvc.WriteRelationship(ctx, assessmentID, model.NewUserSubject(readerID), model.ReaderRelationshipKind)
			Expect(err).To(BeNil())

			// Reader should see assessment
			assessments, err := authzSvc.ListAssessments(ctx, reader)
			Expect(err).To(BeNil())
		...
		It("should list assessments accessible via explicit organization relationship", func() {
			...
			err = authzSvc.WriteRelationship(ctx, assessmentID, model.NewOrganizationSubject(orgID), model.OrganizationRelationshipKind)
			Expect(err).To(BeNil())
	...
	Context("WriteRelationship", func() {
		...
		It("should write owner relationship and grant all permissions", func() {
			...
			// Add new owner
			err = authzSvc.WriteRelationship(ctx, assessmentID, model.NewUserSubject(newOwnerID), model.OwnerRelationshipKind)
			Expect(err).To(BeNil())
		...
		// Test: Validates writing reader relationship
		// Expected: User should gain read permission only
		// Purpose: Tests reader relationship creation
		It("should write reader relationship and grant read permission only", func() {
			ownerID := "write-reader-owner-" + uuid.New().String()[:8]
			readerID := "write-reader-" + uuid.New().String()[:8]
			orgID := "write-reader-org-" + uuid.New().String()[:8]
			assessmentID := uuid.New().String()
			...
			// Verify reader has only read permission
			permissions, err := authzSvc.GetPermissions(ctx, assessmentID, reader)
			Expect(err).To(BeNil())
		...
		It("should return error when writing organization relationship with non-organization subject", func() {
			...
			err = authzSvc.WriteRelationship(ctx, assessmentID, model.NewUserSubject(ownerID), model.OrganizationRelationshipKind)
	...
	Context("DeleteAssessmentRelationship", func() {
		...
		It("should delete specific owner relationship and revoke permissions", func() {
			...
			// Add second owner
			err = authzSvc.WriteRelationship(ctx, assessmentID, model.NewUserSubject(owner2ID), model.OwnerRelationshipKind)
			Expect(err).To(BeNil())
			...
			subject := model.NewUserSubject(owner2ID)
			relationship := model.OwnerRelationshipKind
			err = authzSvc.DeleteAssessmentRelationship(ctx, assessmentID, &subject, &relationship)
			Expect(err).To(BeNil())
		...
		// Test: Validates deleting reader relationship
		// Expected: Reader loses read permission
		// Purpose: Tests reader relationship deletion
		It("should delete reader relationship and revoke read permission", func() {
			ownerID := "delete-reader-owner-" + uuid.New().String()[:8]
			readerID := "delete-reader-" + uuid.New().String()[:8]
			orgID := "delete-reader-org-" + uuid.New().String()[:8]
			assessmentID := uuid.New().String()
			...
			// Verify reader has permission
			permissionsBefore, err := authzSvc.GetPermissions(ctx, assessmentID, reader)
			Expect(err).To(BeNil())
			Expect(permissionsBefore).To(ContainElement(model.ReadPermission))

			// Delete reader relationship
			subject := model.NewUserSubject(readerID)
			relationship := model.ReaderRelationshipKind
			err = authzSvc.DeleteAssessmentRelationship(ctx, assessmentID, &subject, &relationship)
			Expect(err).To(BeNil())

			// Verify reader lost permission
			permissionsAfter, err := authzSvc.GetPermissions(ctx, assessmentID, reader)
			Expect(err).To(BeNil())
		...
		It("should delete organization relationship and revoke org member permissions", func() {
			...
			relationship := model.OrganizationRelationshipKind
			err = authzSvc.DeleteAssessmentRelationship(ctx, assessmentID, &subject, &relationship)
			Expect(err).To(BeNil())
		...
		// Test: Validates DeleteAssessmentAllRelationships removes all relationships
		// Expected: All users lose permissions
		// Purpose: Tests bulk deletion
		It("should delete all assessment relationships via DeleteAssessmentAllRelationships", func() {
			owner1ID := "delete-all-owner1-" + uuid.New().String()[:8]
			owner2ID := "delete-all-owner2-" + uuid.New().String()[:8]
			readerID := "delete-all-reader-" + uuid.New().String()[:8]
			...
			permissionsReader, err := authzSvc.GetPermissions(ctx, assessmentID, reader)
			Expect(err).To(BeNil())
			Expect(permissionsReader).ToNot(BeEmpty())

			// Delete all relationships
			err = authzSvc.DeleteAssessmentAllRelationships(ctx, assessmentID)
			Expect(err).To(BeNil())
			...
			permissionsReaderAfter, err := authzSvc.GetPermissions(ctx, assessmentID, reader)
			Expect(err).To(BeNil())
	...
	Context("GetPermissions", func() {
		...
		// Test: Validates reader has only read permission
		// Expected: Read only
		// Purpose: Tests reader permission retrieval
		It("should return only read permission for reader", func() {
			ownerID := "get-perm-reader-owner-" + uuid.New().String()[:8]
			readerID := "get-perm-reader-" + uuid.New().String()[:8]
			orgID := "get-perm-reader-org-" + uuid.New().String()[:8]
			assessmentID := uuid.New().String()
	...
	Context("GetBulkPermissions", func() {
		...
		It("should return permissions for multiple assessments", func() {
			...
			Expect(err).To(BeNil())
			err = authzSvc.WriteRelationship(ctx, assessment2ID, model.NewUserSubject(ownerID), model.ReaderRelationshipKind)
			Expect(err).To(BeNil())
			...
			// Verify assessment2 permissions (reader)
	...
	Context("HasPermission", func() {
		...
		// Expected: False for reader with delete permission
		...
		It("should return false when user does not have the specified permission", func() {
			...
			readerID := "has-perm-no-reader-" + uuid.New().String()[:8]
			...
			hasPermission, err := authzSvc.HasPermission(ctx, assessmentID, reader, model.DeletePermission)
			Expect(err).To(BeNil())
		...
		It("should correctly check all permission types", func() {
			...
			readerID := "has-perm-all-reader-" + uuid.New().String()[:8]
			...
			readerHasRead, _ := authzSvc.HasPermission(ctx, assessmentID, reader, model.ReadPermission)
			readerHasEdit, _ := authzSvc.HasPermission(ctx, assessmentID, reader, model.EditPermission)
			readerHasShare, _ := authzSvc.HasPermission(ctx, assessmentID, reader, model.SharePermission)
			readerHasDelete, _ := authzSvc.HasPermission(ctx, assessmentID, reader, model.DeletePermission)

			Expect(readerHasRead).To(BeTrue())
			Expect(readerHasEdit).To(BeFalse())
			Expect(readerHasShare).To(BeFalse())
			Expect(readerHasDelete).To(BeFalse())
		})
