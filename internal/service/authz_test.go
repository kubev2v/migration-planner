...
var _ = Describe("Authz Service", Ordered, func() {
	...
	Context("DeletetRelationships", func() {
		...
		It("should delete specific owner relationship and revoke permissions", func() {
			...
			err = authzSvc.DeletetRelationships(ctx,
				model.NewRelationship(assessmentID, model.NewUserSubject(owner2ID), model.OwnerRelationshipKind),
			)
			Expect(err).To(BeNil())
		...
		// Test: Validates DeletetAllRelationships removes all relationships
		// Expected: All users lose permissions
		// Purpose: Tests bulk deletion
		It("should delete all assessment relationships via DeletetAllRelationships", func() {
			...
			err = authzSvc.DeletetAllRelationships(ctx, assessmentID)
			Expect(err).To(BeNil())
