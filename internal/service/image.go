...
func (i *ImageService) GenerateDownloadURL(ctx context.Context, id uuid.UUID) (string, time.Time, error) {
	...
	_, err, _ = i.g.Do(ui, func() (interface{}, error) {
		...
		if err := i.uploadImageToS3(ctx, filename, builder); err != nil {
			return nil, err
		}
...
func (i *ImageService) uploadImageToS3(ctx context.Context, filename string, builder *image.ImageBuilder) error {
	tmpFile, _ := os.CreateTemp("", "upload-*")
	defer func() {
		_ = os.Remove(tmpFile.Name())
	}()

	if err := builder.Generate(tmpFile); err != nil {
		return err
	}
