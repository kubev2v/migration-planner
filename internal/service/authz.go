...
type Authz interface {
	...
	// WriteRelationship creates a relationship (owner, editor, or reader) for an assessment.
	// Acquires: Global Lock (Exclusive) - writes relationships to SpiceDB.
	WriteRelationship(ctx context.Context, assessmentID string, subject model.Subject, relationship model.RelationshipKind) error
	...
	DeleteAssessmentAllRelationships(ctx context.Context, assessmentID string) error
	...
	// Acquires: Global Lock (Exclusive) - writes/deletes relationships to/from SpiceDB.
	DeleteAssessmentRelationship(ctx context.Context, assessmentID string, subject *model.Subject, relationship *model.RelationshipKind) error
...
func (a *AuthzService) InitilizePlatform(ctx context.Context, users map[string][]string) error {
	...
	if err := a.s.Authz().WriteRelationships(ctx, store.WithPlatformRelationship(redhatPlatform, subjectsMap)); err != nil {
		return err
	}
...
// WriteRelationship creates a relationship (owner, editor, or reader) for an assessment.
// Acquires: Global Lock (Exclusive) - writes relationships to SpiceDB.
func (a *AuthzService) WriteRelationship(ctx context.Context, assessmentID string, subject model.Subject, relationship model.RelationshipKind) error {
	ctx, err := a.s.NewTransactionContext(ctx)
	if err != nil {
		return err
	}
	...
	var relationshipsFn model.RelationshipFn
	switch relationship {
	case model.OwnerRelationshipKind:
		relationshipsFn = store.WithOwnerRelationship(assessmentID, subject)
	case model.OrganizationRelationshipKind:
		if subject.Kind != model.Organization {
			return errors.New("failed to write relationship. To write OrganizationRelationshipKind subject must be an organization")
		}
		relationshipsFn = store.WithOrganizationRelationship(assessmentID, subject)
	case model.ReaderRelationshipKind:
		relationshipsFn = store.WithReaderRelationship(assessmentID, subject)
	default:
		return nil
	}

	metrics.IncreaseAuthzTotalRelationshipsMetric(1)
	if err := a.s.Authz().WriteRelationships(ctx, relationshipsFn); err != nil {
		return err
	}
...
func (a *AuthzService) DeleteAssessmentAllRelationships(ctx context.Context, assessmentID string) error {
	return a.DeleteAssessmentRelationship(ctx, assessmentID, nil, nil)
}
...
// If subject and relationship are provided, only that specific relationship is removed (used for unsharing).
// Acquires: Global Lock (Exclusive) - writes/deletes relationships to/from SpiceDB.
func (a *AuthzService) DeleteAssessmentRelationship(ctx context.Context, assessmentID string, subject *model.Subject, relationship *model.RelationshipKind) error {
	ctx, err := a.s.NewTransactionContext(ctx)
	if err != nil {
		return err
	}
	...
	// If no subject is defined, remove all relationships on this assessment
	if subject == nil {
		metrics.IncreaseAuthzTotalRelationshipsMetric(1)
	...
	switch *relationship {
	case model.OwnerRelationshipKind:
		relationshipsFn = store.WithoutOwnerRelationship(assessmentID, *subject)
	case model.OrganizationRelationshipKind:
		relationshipsFn = store.WithoutOrganizationRelationship(assessmentID, *subject)
	case model.ReaderRelationshipKind:
		relationshipsFn = store.WithoutReaderRelationship(assessmentID, *subject)
	default:
		return nil
	}
...
// that performs no authz operations.
...
// ListAssessments is a no-op implementation that returns an empty slice.
func (n *NoopAuthzService) ListAssessments(ctx context.Context, user auth.User) ([]string, error) {
	assessments, err := n.s.Assessment().List(ctx, store.NewAssessmentQueryFilter())
	if err != nil {
		return []string{}, err
	}
...
// WriteRelationship is a no-op implementation that returns nil.
func (n *NoopAuthzService) WriteRelationship(ctx context.Context, assessmentID string, subject model.Subject, relationship model.RelationshipKind) error {
	return nil
}

// DeleteAssessmentAllRelationships is a no-op implementation that returns nil.
func (n *NoopAuthzService) DeleteAssessmentAllRelationships(ctx context.Context, assessmentID string) error {
	return nil
}

// DeleteAssessmentRelationship is a no-op implementation that returns nil.
func (n *NoopAuthzService) DeleteAssessmentRelationship(ctx context.Context, assessmentID string, subject *model.Subject, relationship *model.RelationshipKind) error {
	return nil
}

// GetPermissions is a no-op implementation that returns an empty slice.
func (n *NoopAuthzService) GetPermissions(ctx context.Context, assessmentID string, user auth.User) ([]model.Permission, error) {
	return []model.Permission{}, nil
}

// GetBulkPermissions is a no-op implementation that returns an empty map.
...
// HasPermission is a no-op implementation that returns false.
