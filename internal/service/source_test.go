...
-----END CERTIFICATE-----`
		...
		It("enableProxy false clears proxy fields", func() {
			sourceID := uuid.NewString()
			tx := gormdb.Exec(fmt.Sprintf(insertSourceWithUsernameStm, sourceID, "admin", "admin"))
			Expect(tx.Error).To(BeNil())
			...
			updatedSource, err := s.Source().Get(ctx, uuid.MustParse(sourceID))
			Expect(err).To(BeNil())
			Expect(updatedSource.ImageInfra.HttpProxyUrl).To(BeEmpty())
		...
		It("networkConfigType dhcp clears network fields", func() {
			...
			updatedSource, err := s.Source().Get(ctx, uuid.MustParse(sourceID))
			Expect(err).To(BeNil())
			Expect(updatedSource.ImageInfra.IpAddress).To(BeEmpty())
			Expect(updatedSource.ImageInfra.SubnetMask).To(BeEmpty())
			Expect(updatedSource.ImageInfra.DefaultGateway).To(BeEmpty())
			Expect(updatedSource.ImageInfra.Dns).To(BeEmpty())
		})
