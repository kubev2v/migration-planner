...
var _ = Describe("Source Service", Ordered, func() {
	var (
		s      store.Store
		gormdb *gorm.DB
	)
	...
	Context("update-source", func() {
		It("successfully updates source", func() {
			...
			srv := handlers.NewServiceHandler(service.NewSourceService(s, nil), service.NewAssessmentService(s, nil), service.NewNoopAuthzService(s))
