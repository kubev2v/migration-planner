...
func (o *CollectOptions) Run(ctx context.Context, args []string) error {
	...
	log.Printf("🔍 Starting collector; dataDir=%q, creds=%q", o.config.DataDir, o.credentialsFilePath)
...
func (o *CollectOptions) useCmdCredentials() error {
	...
	if hasURL || hasUser || hasPass {
		...
		log.Printf("using credentials from CMD")
	} else {
		log.Printf("no credentials provided via CMD, skipping")
		return nil
	}
